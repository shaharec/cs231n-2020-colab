...
def compute_saliency_maps(X, y, model):
    ...
    with tf.GradientTape() as t:
      t.watch(arg)
      scores = model.call(arg)
      correct_scores = tf.gather_nd(scores, tf.stack((tf.range(N), y), axis=1))
      loss = tf.nn.softmax_cross_entropy_with_logits(y,correct_scores)
    grad = t.gradient(loss,arg)
    saliency = np.abs(grad).max(axis=3)
...
def make_fooling_image(X, target_y, model):
    ...
    X_fooling = tf.convert_to_tensor(X_fooling, dtype=tf.float32)
    ...
    for i in range(100): 
      ...
      with tf.GradientTape() as t:
        ...
        if np.argmax(score) == target_y:
          break
        correct_score = score[0,target_y]
      grad = t.gradient(correct_score,X_fooling)
      dX = learning_rate * grad / tf.norm(grad)
      #preform gradient assent
      X_fooling += dX
      del t
...
def class_visualization_update_step(X, model, target_y, l2_reg, learning_rate):
    ...
    with tf.GradientTape() as t:
      ...
      score = model.call(X)
      correct_score = score[0,target_y]
    grad = t.gradient(correct_score,X)
    dX = learning_rate * grad / tf.norm(grad)
    #preform gradient assent
    X += dX
    del t
